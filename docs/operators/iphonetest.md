...
- Upstream slash-command:
  `Lifecycle-Innovations-Limited/healify` → `.claude/commands/iphonetest.md`
  ([file](https://github.com/Lifecycle-Innovations-Limited/healify/blob/main/.claude/commands/iphonetest.md))
...
Farm from a Claude session. It auto-resolves the latest Healify staging (or
...
Source-of-truth: `healify/.claude/commands/iphonetest.md`.

## 2. Modes

| `--mode` | Shape | When to use |
|---|---|---|
| `tunnel` **(default since [PR #6172](https://github.com/Lifecycle-Innovations-Limited/healify/pull/6172))** | Interactive REPL or JSON-RPC over stdio; one session held open across many commands | An operating Claude driving the device turn-by-turn |
| `walkthrough` | Autonomous loop with the built-in Healify-QA goal (visit every tab, screenshot each) | Smoke a fresh build |
...
#   → expect: Healify-iOS-E2E

# 2. Doppler secrets present
doppler secrets --project healify --config dev_local --only-names \
...
| `cmd` | App-rendered Healify UI | iOS system UI (Springboard / Spotlight) |
|---|---|---|
| `tap` (coords) | ✓ | ✓ |
...
the screenshot shows the iOS home screen — not the Healify app.
...
Healify staging IPA, that is iPhone 17 Pro on iOS 26.3.1.

Shipped in
[PR #6171](https://github.com/Lifecycle-Innovations-Limited/healify/pull/6171)
...
```bash
doppler run --project healify --config dev_local -- \
  python3 scripts/devicefarm/stop-session.py --all-active
```
...
**Symptom:** `doppler run --project healify --config dev_local -- bash -c '... python3 tunnel.py --json ...'` either never receives the first JSON command or drops stdin midway.
...
doppler secrets download \
  --project healify --config dev_local \
...
cat "$TMP/in" | \
  python3 ~/Projects/healify-workspace/healify/scripts/devicefarm/iphonetest.py \
    --mode tunnel --json \
  > "$TMP/out.log" 2>&1 &
echo $! > "$TMP/pid"
```
...
```bash
python3 ~/Projects/healify-workspace/healify/scripts/devicefarm/list-sessions.py
```
...
When you hit one of these in the field, file in the healify repo and link
