...
- Driver script: `my-project/scripts/devicefarm/iphonetest.py` → hands off to
  ...
  [`tunnel.py`](https://github.com/your-org/my-project/blob/main/scripts/devicefarm/tunnel.py).
...
with coords) then re-issue `type`. This is how My-Project staging text inputs
accept input.
...
`bundleId="com.staging.my-project"`) would eliminate this dance. File against
my-project under `scripts/devicefarm` — pure additive change. Until that lands,
...
# 3. Tap My-Project icon (coords read from 02.png)
...
- **`launch_app` / `activate_app` JSON command in `tunnel.py`.** Wraps
  `mobile: activateApp` with `bundleId="com.staging.my-project"` (or the
...
My-Project staging IPA. When anything in §5 / §6 changes, update this file and
