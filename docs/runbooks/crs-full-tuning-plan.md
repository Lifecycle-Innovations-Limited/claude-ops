...
> Linear: [HEA-4672](https://linear.app/lifecycle-innovations/issue/HEA-4672) · Owner: <OWNER_NAME> · Created: 2026-06-19
>
...
| Account pool | ~10 Claude **Max OAuth** accounts: `pool-aurora/<OWNER_BRAND>/<USER>/chairman/foundation/<OWNER_BRAND>-personal/<OWNER_BRAND>-team` + `canary-lifecycle/support/sponsors`. Records: Redis hash `claude:account:<uuid>` |
...
- Verify the dead account: `pool-<USER>` (`a9f4806a…`) was the out_of_credits trigger;
...
3. Top-up status of `pool-<USER>` (billing-dead account) — keep parked vs. top up vs. remove from pool.
