...
| `doppler_token`              | Optional | `""`        | Doppler service or personal token for the `@dopplerhq/mcp-server` MCP integration. Generated by `/ops:setup` or paste manually. |
| `doppler_project`            | Optional | `""`        | Default Doppler project slug (e.g. `my-app`). Used by the Doppler MCP server.                                                   |
| `doppler_config`             | Optional | `""`        | Default Doppler config name (e.g. `dev`, `stg`, `prd`). Used by the Doppler MCP server.                                         |

**Sensitive fields** (`telegram_api_id`, `telegram_api_hash`, `telegram_phone`, `telegram_session`, `klaviyo_private_key`, `meta_ads_token`, `shopify_admin_token`, `shipbob_access_token`, `bland_ai_api_key`, `elevenlabs_api_key`, `groq_api_key`, `stripe_secret_key`, `revenuecat_api_key`, `doppler_token`) are marked `"sensitive": true` in the schema and are encrypted at rest by Claude Code.
...
1. **Doppler MCP Server** — if `doppler_token` is configured in userConfig, the `@dopplerhq/mcp-server` MCP server runs automatically, giving Claude direct tool access to Doppler secrets without shelling out.
2. **Doppler CLI** — if `doppler` CLI is installed and configured, `/ops:setup` will link it. Skills read secrets via `doppler run --` or environment injection. Used as fallback when MCP is unavailable.
3. **Password manager** — 1Password (`op`), Dashlane (`dcli`), or Bitwarden (`bw`) vaults are scanned by `/ops:setup` during credential collection.
4. **macOS Keychain** — the Telegram wizard stores `api_id`, `api_hash`, `phone`, and `session` in the system keychain automatically.
5. **Plugin `userConfig`** — values set in Claude Code plugin settings (see Plugin Settings section above).
6. **Shell environment** — variables exported in your shell profile (e.g. `~/.zshrc`).
