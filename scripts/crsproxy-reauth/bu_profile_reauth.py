...
def _fallback_email_entry(proc, client, session_id, provider, email,
    ...
    safe = safe_email(email)
    port = meta["callback_port"]
    # Use GMAIL_TARGET_INBOX (where emails are forwarded to) for Gmail polling,
    # not the account email itself. Falls back to email if env var not set.
    gog_account = os.environ.get("GMAIL_TARGET_INBOX", email)
    ...
    if not needs_code:
        ...
        # Claude's default flow is magic link ("click the link sent to").
        # Try magic link first, then fall back to verification code.
        ...
        if magic:
            ...
            task_magic = (
                ...
                "If you see an Authorize or Allow button, click it. "
                "Report what you see."
            )
            ...
            # Check if we got the callback or need to click Authorize
            callback_url = extract_callback_url(result_magic, client, provider)
            if callback_url:
                log(f"[FALLBACK] Callback URL from magic link: {sanitize_url(callback_url)}")
                return _complete_reauth(proc, callback_url, auth_file,
                                        stale_backup, email, provider, meta)

            if detect_captcha(result_magic, client):
                log("[CAPTCHA] Captcha detected after magic link")
                callback_url, session_id = handle_captcha_checkpoint(
                    client, run_magic.get("id", ""), session_id, provider, email, port)
                if callback_url:
                    return _complete_reauth(proc, callback_url, auth_file,
                                            stale_backup, email, provider, meta)
                return EXIT_CAPTCHA
        else:
            log("[FALLBACK] No magic link — trying verification code...")
