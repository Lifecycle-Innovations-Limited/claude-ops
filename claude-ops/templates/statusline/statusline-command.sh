...
# Local-proxy guard: a stale CLAUDE_CODE_USE_BEDROCK=1 can linger in the process
# env even though the session actually routes through a local proxy on loopback
# (CLIProxyAPI, or a leftover relay on an upgraded machine). In that case the
# real provider is the pooled OAuth account, not Bedrock. Force "max" and
# overwrite any stale cache so a session that briefly cached "bedrock" at startup
# (before proxy routing settled) never keeps showing Bedrock.
billing_cache="${CACHE_DIR}/claude-billing-${session_id}"
_is_local_proxy=""
case "${ANTHROPIC_BASE_URL:-}" in
  http://127.0.0.1:*|http://localhost:*|http://[::1]:*) _is_local_proxy=1 ;;
esac
if [ "${CLAUDE_CODE_USE_BEDROCK:-}" = "1" ] && [ -z "$_is_local_proxy" ]; then
  sub_type="bedrock"
elif [ -n "$_is_local_proxy" ]; then
  sub_type="max"
