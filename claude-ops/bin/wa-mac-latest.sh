...
# Env: WA_MAC_SSH  ssh target (REQUIRED, e.g. user@mac-host)
set -euo pipefail
MAC="${WA_MAC_SSH:?set WA_MAC_SSH=user@mac-host (Tailscale ssh target)}"
...
raw=$(timeout 35 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "$MAC" "sqlite3 -separator $'\x1f' \"$DB\" \"$SQL\"" 2>&1) || { echo "ERROR: could not reach Mac WhatsApp DB ($MAC). $raw" >&2; exit 2; }
if [ "$JSON" = 1 ]; then
