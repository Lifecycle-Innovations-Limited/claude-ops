---
name: ops-fleet
description: Read-only fleet dashboard — every Claude session (local + remote) with token type (oauth/api/bedrock/cliproxy), CLIProxyAPI relay state, account utilization, and per-session detail. No actions.
...
`claude daemon status`, the account-rotation daemon state, and the CLIProxyAPI relay
(legacy CRS-compatible deployments included). It renders, it never mutates.
...
- **CLIProxyAPI relay**: health at `http://127.0.0.1:${CRS_PORT:-3005}/health`, legacy allowlist at
...
(`scripts/install-account-rotator-linux.sh`). If the rotator isn't installed the CLIProxyAPI /
...
- Override the CLIProxyAPI-compatible relay port with `CRS_PORT=NNNN` (legacy env name; default 3005; 3000 is the legacy port alias).
...
total sessions + token-type breakdown, anything blocked or flapping (high `att:`), CLIProxyAPI
relay health, and any account ≥90% util. The dashboard is the deliverable; the summary is the footnote.
