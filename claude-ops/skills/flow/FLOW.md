...
| **build**         | project → **GSD** `gsd-execute-phase` (fresh-context waves) / `gsd-manager` (multi-project orchestrator, ≤5 subagents/action); ad-hoc → direct edits + worktree | optional overlay: **feature-dev** `/feature-dev` (explore → architect → implement → review); worktree isolation (mandatory) | `ops:ops-orchestrate`, `superpowers:dispatching-parallel-agents`, personal `subagents`/`orchestrator` |
...
| **project-state** | **GSD `.planning/` — SOLE OWNER**; `ops:ops-projects` is the read-only dashboard over it (via the `ops-gsd-registry-sync.sh` cron)                                        | `ops:ops-linear` syncs phases→Linear                                 | personal `my-project-gsd`                                                                             |
...
| pdf, export                                               | docs          | gstack `/make-pdf`                                                                                   |
| scrape, extract, pull-data                                | web           | gstack `/scrape` (codify a repeat flow with `/skillify`)                                             |
| save-context, resume, park                                | context       | gstack `/context-save` / `/context-restore`; **project** → `gsd-pause-work` / `gsd-resume-work`       |
| freeze, guard, scope-edits                                | safety        | gstack `/freeze` (dir scope), `/guard` (full), `/careful` (destructive-cmd warnings), `/unfreeze`     |
| design-review, ux-audit                                   | review        | gstack `/design-review`; iOS → `/ios-design-review`; devex → `/devex-review`                          |
| ios-fix, ios-clean, ios-sync                              | ios           | gstack `/ios-fix`, `/ios-clean`, `/ios-sync`                                                          |
| benchmark, perf, regression                               | test          | gstack `/benchmark`                                                                                  |
