...
```bash
# Install gog (private example/gog CLI — try npm/bun first, fall back to source)
npm install -g @example/gog 2>/dev/null || \
bun install -g @example/gog 2>/dev/null || \
(git clone https://github.com/example/gog ~/.gog && cd ~/.gog && ./install.sh) || \
{ echo "Could not install gog. Options:"; \
  echo "  1. Ask Sam (internal team) for the latest release binary"; \
  echo "  2. Request access to github.com/example/gog"; \
