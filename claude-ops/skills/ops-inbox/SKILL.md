...
- **When to use:** if `wa-inbox-fresh.sh` reports the store is **STALE** (exit 2, or "newest message" far older than expected), OR a contact's *known* reply is missing from the bridge (common on `@lid` chats), cross-check the Mac app's ground-truth store before classifying that thread as "no reply".
- **Command:** `bin/wa-mac-latest.sh --contact <name|number> [N]` (also `--recent [N]`, `--since "YYYY-MM-DD HH:MM"`, add `--json` for machine-readable output). It reads `~/Library/Group Containers/group.net.whatsapp.WhatsApp.shared/ChatStorage.sqlite` over Tailscale SSH (target via `WA_MAC_SSH`, e.g. `user@mac-host`). Schema: `ZWAMESSAGE` (`ZTEXT`, `ZISFROMME`, `ZMESSAGEDATE` = seconds since 2001-01-01) joined to `ZWACHATSESSION` (`ZPARTNERNAME`, `ZCONTACTJID`).
- **READ-ONLY ground truth.** This reader never writes and never sends. Sends still go through the whatsmeow bridge (`mcp__whatsapp__send_message`) under the Rule-6 outbound-approval gate — the Mac store is only consulted to confirm what actually arrived.
