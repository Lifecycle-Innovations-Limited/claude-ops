...
# message-ID was scanned from one account (e.g. user@gmail.com) but the
# send defaults to a DIFFERENT account (e.g. sam@example.com): the same thread
...
for e in ("sam@example.com", "user@gmail.com"):
    if e not in seen:
