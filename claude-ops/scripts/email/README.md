...
message-ID was scanned from one Gmail account (e.g. `user@gmail.com`) but
the send defaults to a **different** account (e.g. `sam@example.com`) — the same
...
| `--account <acct>` | Pin a gog account. If omitted, **auto-discovers** (enumerates oauth gmail accounts via `gog auth list`, always also probing `sam@example.com` and `user@gmail.com`). |
