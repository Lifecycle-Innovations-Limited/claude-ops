#!/usr/bin/env bash
# Install a macOS launchd SSH tunnel: local CLIProxyAPI/legacy CRS-compatible relay port → remote relay host.
# Set CRS_TUNNEL_SSH_HOST (or legacy CRS_FRA_SSH_HOST) to your SSH target, e.g. user@relay.example.com

set -euo pipefail

if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "skip: CLIProxyAPI/legacy relay remote tunnel is macOS-only (launchd)" >&2
  exit 0
fi
...
if [[ -z "$SSH_HOST" ]]; then
  echo "error: set CRS_TUNNEL_SSH_HOST (legacy env name; SSH target for remote relay, e.g. user@host)" >&2
  exit 1
fi
