...
Keychain rotation swaps one account at a time. To spread load across several
accounts at once, run [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)
locally. It holds one OAuth seat file per account and answers on a local port.

`rotate.mjs` already writes CLIProxyAPI seat files as part of a rotation, so the
proxy and the rotator stay in sync without a separate feed. Point clients at the
proxy with `CLIPROXYAPI_BASE_URL`, inspect the pool with `/ops:ops-fleet`, and
set up seats with `/ops:ops-rotate-setup`.

CLIProxyAPI is the only supported multi-account path. The earlier relay backend
(claude-relay-service) has been removed: it required a static bearer token
injected into `settings.json` for every session, which is exactly the credential
handling this plugin should not do. If a machine still has a
`~/.claude/crs-session-settings.json` overlay or a `cr_` token in
`settings.json`, run `claude-stack route --mode oauth` once to clear it;
`claude-stack doctor` reports it as `legacy_relay_env` until you do.

See `config.example.json` → `rotation` block for the rotator's own tunables. A
config that still has a `crs` block keeps working: it is read as the legacy name
for the same block.
...
| `rotate-magic.mjs`          | Thin entry → `rotate.mjs --magic-link` (standalone reauth).               |
