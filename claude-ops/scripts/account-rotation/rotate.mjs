...
function pickNextAccount(config, state, liveUtil = {}) {
  ...
  for (const pool of [refreshableNormal, refreshableLow, manualReauthNormal, manualReauthLow, euNormal, euLow]) {
    ...
    for (const candidates of [fresh, exhausted]) {
      ...
      // (Sam 2026-05-30: never rotate TO / use sam@samfeldt.com once it's ≥50% on max(5h,7d).)
...
//   `support@healify.ai`            → `Healify Support`
//   `info@auroracapital.nl`         → `Aurora Capital`
//   `heartfeldt`                    → `Heartfeldt`
//   `sam@heartfeldt.foundation`     → `Heartfeldt Foundation`
function displayNameFor(account) {
  ...
  // For sam@samfeldt.com → "Samfeldt"
  // For info@auroracapital.nl → "Aurora Capital"
  // For support@healify.ai → "Healify Support"
