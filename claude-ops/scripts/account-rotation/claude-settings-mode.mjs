...
export function clearHardcodedModelsForOAuthClaudeSettings() {
  const s = readSettings();
  const env = s.env && typeof s.env === 'object' ? { ...s.env } : {};
  // Raw OAuth means no base-URL override and no injected key: Claude Code uses
  // its own credential. A pooled setup routes through CLIProxyAPI, which owns
  // the seat files rather than an env pair inside settings.json.
  for (const k of [
