...
|---|---|---|---|
| `info@example.com` | _(none — key = email)_ | personal | autoAuthDisabled |
| `support@my-project.example.com` | _(none — key = email)_ | personal | |
| `user@gmail.com` | _(none — key = email)_ | personal | |
| `sam@example.com` | _(none — key = email)_ | personal | maxUtilPercent=50 |
| `sam@account-main.org` | `account-team` | Account-Main (team org d38d677e) | |
| `sam@account-main.org` | `account-main` | sam@account-main.org's Organization (personal c2002323) | |
| `info@lifecycleinnovations.limited` | _(none — key = email)_ | personal | |
| `chairman@account-main.org` | _(none — key = email)_ | Account-Main team seat | |
| `sponsors@account-main.org` | _(none — key = email)_ | Account-Main team seat | |
| `sam@account-main.foundation` | _(none — key = email)_ | personal | |

### Vault key format
`Claude-Rotation-<accountKey>` — e.g.:
- `Claude-Rotation-account-team`
- `Claude-Rotation-account-main`
- `Claude-Rotation-user@gmail.com`

### Stale key migration
`account-personal` was an old label that appeared in logs and state.json.  
...
`account-personal` → `account-main` (silently, one-time, then writes the corrected state).
...
**File:** `daemon.mjs`  
The rescue log line used `taggedEmail` (raw email address, e.g. `sam@account-main.org`) instead of the canonical `probeKey` (label, e.g. `account-team`). Changed to `${probeKey} (${taggedEmail})` so the canonical key is always primary and the email is context.
