...
| `POST /api/v1/media`          | `{"filename":"qr.png","content_base64":"…"}` | `{success,media_path,bytes}`     | **Stage a file on the bridge host** and get back a path for `send_file`. Token-gated (`WA_API_TOKEN`), unlike every `/api/*` route. 64 MiB decoded cap, content-addressed, written `0600` into `store/uploads/`. |
...
| `upload_media {filename,content_base64}`                                                                                    | → `POST /api/v1/media`               | stage bytes on the bridge host, then pass the returned `media_path` to `send_file` |
...
2. **SENDS (text/media)** → `mcp__whatsapp__send_message` **if** the :8090 proxy loaded the tools;
   ...
   **Attachments cross a host boundary.** `send_file` reads `media_path` off the **bridge's own
   ...
   its own token. Pinned by `tests/outbound-guard/test-hook-matrix.py`.
