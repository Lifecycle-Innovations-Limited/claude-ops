...
def load_park_rules() -> list[dict]:
    ...
    if not PARK_RULES.exists():
        default = [
            {
                "id": "my-project-prod-fires-owned-by-orchestrator",
                "match_any": [
                ...
                "reason": "My-Project prod/staging fires owned by orchestrator in dedicated worktree (2026-05-25). Do not auto-spawn duplicate workers.",
            },
        ]
        if not DRY_RUN:
